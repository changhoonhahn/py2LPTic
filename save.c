...
void write_density_field_data(void)
{
  ...
  for(groupTask = 0; groupTask < nprocgroup; groupTask++)
	...
	{ 
	  ...
	  sprintf(buf, "%s/Coordinates_ptype_%d.%d", OutputDir, Type_df, ThisTask);
	  fd = fopen(buf, "w");
	  ...
	  my_fwrite(coordinates, sizeof(long long), Local_nx*Nmesh*(Nmesh/2 + 1), fd);
	  fclose(fd);
      
	  sprintf(buf, "%s/Amplitudes_ptype_%d.%d", OutputDir, Type_df, ThisTask);
	  fd = fopen(buf, "w");
	  my_fwrite(&NTaskWithN, sizeof(int), 1, fd); //save the number of subfiles
	  my_fwrite(&Nmesh,      sizeof(int), 1, fd); //save Nmesh
	  my_fwrite(&Local_nx,   sizeof(int), 1, fd); //save Local_nx
	  ...
	  sprintf(buf, "%s/Phases_ptype_%d.%d", OutputDir, Type_df, ThisTask);
	  fd = fopen(buf, "w");
	  my_fwrite(&NTaskWithN, sizeof(int), 1, fd); //save the number of subfiles
	  my_fwrite(&Nmesh,      sizeof(int), 1, fd); //save Nmesh
	  my_fwrite(&Local_nx,   sizeof(int), 1, fd); //save Local_nx
