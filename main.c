...
void displacement_fields(void)
{
  ...
  double delta, phase, phase_shift, ampl, hubble_a;
  double u, v, w;
  ...
  for(Type = MinType; Type <= MaxType; Type++)
	...
	for(j = 0; j < Nmesh; j++)
	  for(k = 0; k <= Nmesh / 2; k++)
		...
		if (axes==0)
		  {
		    coordinates[(i * Nmesh + j) * (Nmesh / 2 + 1) + k] = ((i+Local_x_start) * Nmesh + j) * (Nmesh / 2 + 1) + k;
